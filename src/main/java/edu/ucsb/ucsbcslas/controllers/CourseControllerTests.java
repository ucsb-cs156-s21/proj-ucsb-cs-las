...
public class CourseControllerTests {
  ...
  @Test
  public void testShowTutor() throws Exception {
    List<Tutor> expectedTutors = new ArrayList<Tutor>();
    expectedTutors.add(new Tutor(1, 'Andrew', 'Lu', 'alu@ucsb.edu'));
    when(mockTutorRepository.findAll()).thenReturn(expectedTutors);
    MvcResult response = mockMvc.perform(get(L"/api/public/tutors/1").contentType("application/json")
        .header(HttpHeaders.AUTHORIZATION, "Bearer " + userToken())).andExpect(status().isOk()).andReturn();
